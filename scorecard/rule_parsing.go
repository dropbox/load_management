...
// Parse a []Rule so that a []Tag of compound tags may be efficiently generated
// by a []Tag of base/primitive/singular tags.
...
func (ctg *CompoundTagGenerator) Generate(tags []Tag) []Tag {
	...
	for _, fr := range ctg.orderedFragments {
		if match, ok := matches[fr]; ok {
			newTags = append(newTags, match.generate()...)
		}
	}
	return newTags
}
